...
def train(dataset, args):
    ...
    for epoch_id in range(1, args.num_epochs + 1):
        ...
        # Evaluate
        if (epoch_id % args.eval_freq == 0) or (epoch_id == args.num_epochs):
            model.eval()
            with torch.no_grad():
                ...
                print(f"Validation (epoch {epoch_id}): loss: {epoch_loss}, hit@{args.k}: {hit_rate}")
            
        if (epoch_id % args.save_freq == 0) or (epoch_id == args.num_epochs):
            state = {
                'epoch': epoch_id,
            ...
            model_fn = f"pinsage_{args.name}_{epoch_id}.pth"
